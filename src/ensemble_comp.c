...
static void calc_eta_res(real *eta, t_atoms *atoms, eta_res_t *eta_res);
...
void res_pdb(const char *pdb_fname, real *eta, int natoms, eta_res_t *eta_res) {
	...
	// TODO: use init_atoms, eliminate natoms parameter
	...
	calc_eta_res(eta, &atoms, eta_res);
...
#define NATOMS
	...
	// done_top(&top); // Causes segmentation fault. See implementation in typedefs.c

	print_log("%d residues\n", top.atoms.nres);
	for(i = 0; i < top.atoms.nr; i++) {
		print_log("Atom %d Residue %d\n", i + 1, top.atoms.atom[i].resind);
	}
...
void res_tpx(const char *tpr_fname, real *eta, eta_res_t *eta_res) {
	// t_tpxheader header;
	t_inputrec ir;
	gmx_mtop_t mtop;
	matrix box;
	...
	calc_eta_res(eta, &(mtop.moltype->atoms), eta_res);
